...
if ($id < 1 || $name === '' || $time === '' || $duration === '' || !is_array($days) || count($days) === 0) {
...
$stmt = $db->prepare('UPDATE courses SET name = ?, description = ?, days = ?, time = ?, duration = ? WHERE id = ?');
$stmt->bind_param('sssssi', $name, $description, $daysJson, $time, $duration, $id);
$stmt->execute();

json_response(true, 'Kurs yangilandi.');
